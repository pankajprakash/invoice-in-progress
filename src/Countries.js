const countryCode = [
    ...
    { name: "land Islands", dial_code: null, code: "AX" },
  ...
  const sortCountries = () => {
    ...
    let US = countries.find(item => item.code === "US");
    countries.sort((a, b) => a.name.localeCompare(b.name));
    countries = countries.filter(item => item.code !== 'US');
    return [US, ...countries];
  }
  
  export const COUNTRY_CODE_LIST = sortCountries().map((item, i) => ({
    label: `${item.dial_code} (${item.code})`,
    value: item.dial_code
  }));
  
  export const COUNTRY_LIST = sortCountries().map(item => {
    return {
      label: item.name,
      value: item.code,
